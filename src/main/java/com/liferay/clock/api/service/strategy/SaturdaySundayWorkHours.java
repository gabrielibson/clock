...
/**
 * Saturday and Sunday work hours strategy class. Implements
 * {@link WorkHoursCalculationStrategy} interface
 * 
 * @author Gabriel
 *
 */
public class SaturdaySundayWorkHours implements WorkHoursCalculationStrategy {
	...
	/**
	 * Saturday and Sunday work hours calculation strategy. Used when a work starts
	 * on Saturday and ends on Sunday.
	 * 
	...
	public Duration calculateWorkHours(LocalDateTime time1, LocalDateTime time2) {
		...
		Double timeSaturday = Duration
				.between(time1, LocalDateTime.of(time1.toLocalDate(), LocalTime.from(endSaturdayTime))).plusMinutes(1)
				.toMinutes() * multiplicative_factor_saturday;
		Double timeSunday = Duration
				.between(LocalDateTime.of(time2.toLocalDate(), LocalTime.from(startSundayTime)), time2).toMinutes()
